...
[stated0,y0,check0,xRel0,nRel0,dndtRel0] = hux_tutorial_kinematic_quasi(0,state0,parms);

tSpan=[0 t_end]; % chop up time to save memory (reduce state vector)
ode_fun=@(t,state)hux_tutorial_kinematic_quasi(t,state,parms);
...
for i=1:length(t)
    [stated(:,i),y(:,i),check(:,i),x,n,dndt] = hux_tutorial_kinematic_quasi(t(i),state(i,:)',parms);
