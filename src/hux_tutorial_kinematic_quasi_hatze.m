<<<<<<< HEAD:hux_tutorial_kinematic.m
function [ dstatedt,varargout ] = hux_tutorial_kinematic( t,state,parms )
=======
function [ dstatedt,varargout ] = hux_tutorial_kinematic_quasi_hatze( t,state,parms )
>>>>>>> force_rate:hux_tutorial_kinematic_quasi_hatze.m
...
state=state(:);
n = state(1:end-2); % [] fraction of bound cross bridges
gamma = state(end-1); % [] relative free Ca2+ concentration
lce = state(end); % [m] contractile element length
...
lmtcd=0;
=======
lmtc=parms.lmtc0+0.2*lceopt*sin(t);
lmtcd=0.2*lceopt*cos(t);
>>>>>>> force_rate:hux_tutorial_kinematic_quasi_hatze.m
