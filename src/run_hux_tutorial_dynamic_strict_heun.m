...
[stated0,y0,check0,xRel0,nRel0,dndtRel0] = hux_tutorial_dynamic_strict(0,state,parms);
t_end=1; % [s] simulation time, starting at t=0 ...

% do integration using heun algorithm
% initialization and stepsize
dt=.0001; % 1 ms stepsize
t=0:dt:t_end;
% initialize output matrices 
y=zeros(length(t),length(y0));
relStates=zeros(length(t),3);

tic
for  k=1:length(t)
    % first step (euler)
    [stated_euler,y(k,:)]=hux_tutorial_dynamic_strict(t(k),state,parms); % stated at t (euler)
    state_euler=state+stated_euler*dt; % euler estimate of state at t+dt
    
    % grab relevant variables from state and stated
    relStates(k,:)= state(end-2:end); % [m]
    
    % second step (heun)
    stated_heun=hux_tutorial_dynamic_strict(t(k)+dt,state_euler,parms); % improved estimate of stated at t=dt
    state=state+(stated_heun+stated_euler)/2*dt; % new state estimate based on average of euler estimate and heun estimate    
end
toc
disp(['nr of func evals: ',num2str(2*length(t))])
