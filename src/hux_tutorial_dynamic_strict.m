function [ dstatedt,varargout ] = hux_tutorial_strict( t,state,parms )
...
state=state(:);
n = state(1:end-3); % [] fraction of bound cross bridges
gamma = state(end-2); % [] relative free Ca2+ concentration
...
%% complete stated
dstatedt = [dndt; gammad; lmtcd; lmtcdd];
...
if nargout > 1 || err == true
    ...
    varargout{1}=[stim q Fce Fpe Fse fisomrel p_cb p_act lce];
    if nargout>2
        varargout{2}=xRel;        
        if nargout>3
            varargout{3}=nRel;            
