...
lmtc0=(1+se_strain)*parms.lse_slack+1.1*parms.lceopt; % [m]
lmtcd0=0; % [m/s]
...
[stated0,y0,check0,xRel0,nRel0,dndtRel0] = hux_tutorial_dynamic_quasi(0,state0,parms);
...
ode_fun=@(t,state)hux_tutorial_dynamic_quasi(t,state,parms);
...
for i=1:length(t)
    [stated(:,i),y(:,i),check(:,i),x,n,dndt] = hux_tutorial_dynamic_quasi(t(i),state(i,:)',parms);
