function [ jac ] = hux_tutorial_kinematic_strict_jac2( t,state,parms )
...
[fx,gx]=rateFun(x(:)); % see function for details
...
% dndt wrt n
dndt_wrt_n=-(fx+gx);
row_idx_dndt_wrt_n=idx(1:end-1);
col_idx_dndt_wrt_n=idx(1:end-1);
...
row_idx_dndt_wrt_gamma=idx(1:end-1);
col_idx_dndt_wrt_gamma=ones(1,N-1)*N(end);
